...
(() => {
  ...
  async function fetchProgressOnce() {
    ...
    try {
      ...
      const messageLine = p.message ? `Message: ${p.message}` : "";
      ...
      const portsLine =
        (p.ports_done != null && p.port_total)
          ? `Ports ${p.ports_done}/${p.port_total} (last ${p.last_port != null ? p.last_port : "-"})`
          : "";
      ...
      //const lines = [timeLine, statusLine, messageLine, portsTestedLine, portsLine, summaryLine, completedLine]
  ...
  async function stopScan() {
    ...
    // Abort UI fetch immediately
    if (scanFetchAbort) {
      try { scanFetchAbort.abort(); } catch {}
    }
